...
def ParseRecord(expr):
  word, aoa, trans = expr.split("\t")
  aoa = float(aoa)
  trans = trans.split(",")
  return (word, aoa, trans)
...
def GetCandidates(records, used_words):
  ...
  candidates = []
  num_tries = 0
  while len(candidates) < NUM_CANDIDATES and num_tries < 20:
    num_tries += 1
    rank, expr = random.choice(records)
    word, aoa, trans = ParseRecord(expr)
    ...
    for tran in trans:
      ...
      if kana_tran:
        if top_letter == "a":
          if regex.search(r"^[アエ]", kana_tran): continue
        elif top_letter == "b":
          if regex.search(r"^[バビブベボ]", kana_tran): continue
        elif top_letter == "c":
          if regex.search(r"^[カキクケコサシスセソチ]", kana_tran): continue
        elif top_letter == "d":
          if regex.search(r"^[ダヂヅデド]", kana_tran): continue
        elif top_letter == "e":
          if regex.search(r"^[エイ]", kana_tran): continue
        elif top_letter == "f":
          if regex.search(r"^[ハヒフヘホ]", kana_tran): continue
        elif top_letter == "g":
          if regex.search(r"^[ガギグゲゴジ]", kana_tran): continue
        elif top_letter == "h":
          if regex.search(r"^[ハヒフヘホ]", kana_tran): continue
        elif top_letter == "i":
          if regex.search(r"^[イア]", kana_tran): continue
        elif top_letter == "j":
          if regex.search(r"^[ジヤユヨイエ]", kana_tran): continue
        elif top_letter == "k":
          if regex.search(r"^[カキクケコ]", kana_tran): continue
        elif top_letter == "l":
          if regex.search(r"^[ラリルレロ]", kana_tran): continue
        elif top_letter == "m":
          if regex.search(r"^[マミムメモ]", kana_tran): continue
        elif top_letter == "n":
          if regex.search(r"^[ナニヌネノ]", kana_tran): continue
        elif top_letter == "o":
          if regex.search(r"^[アウオ]", kana_tran): continue
        elif top_letter == "p":
          if regex.search(r"^[パピプペポ]", kana_tran): continue
        elif top_letter == "q":
          if regex.search(r"^[ク]", kana_tran): continue
        elif top_letter == "r":
          if regex.search(r"^[ラリルレロ]", kana_tran): continue
        elif top_letter == "s":
          if regex.search(r"^[サシスセソ]", kana_tran): continue
        elif top_letter == "t":
          if regex.search(r"^[タチツテトデ]", kana_tran): continue
        elif top_letter == "u":
          if regex.search(r"^[ウアユ]", kana_tran): continue
        elif top_letter == "v":
          if regex.search(r"^[バビブベボヴ]", kana_tran): continue
        elif top_letter == "w":
          if regex.search(r"^[ウワヴ]", kana_tran): continue
        elif top_letter == "x":
          if regex.search(r"^[ザジズゼゾ]", kana_tran): continue
        elif top_letter == "y":
          if regex.search(r"^[ヤユヨイエ]", kana_tran): continue
        elif top_letter == "z":
          if regex.search(r"^[ザジズゼゾ]", kana_tran): continue
      good_trans.append(tran)
      if len(good_trans) >= NUM_SHOWN_TRANS: break
    if not good_trans: continue
    sel_trans = ",".join(sorted(good_trans))
    if sel_trans in checked_trans: continue
...
<body>
""", end="")
  P('<h1><a href="{}">英単語年齢診断</a></h1>', script_url)
  if result_name and user_name and history:
    ...
    for rank, score in history:
      ...
      if score:
        hist_aoas.append(record[1])
      record_scores.append((rank, record, score))        
    hist_aoas = sorted(hist_aoas, reverse=True)
    if hist_aoas:
      aoa = round(hist_aoas[min(2, len(hist_aoas) - 1)])
      if len(hist_aoas) < 3:
        aoa -= 3 - len(hist_aoas)
    else:
      aoa = 0
    aoa = min(max(3, aoa), 20)
    ...
    P('</ul>')
    tweet_msg = "英語力年齢診断: {}さんの語彙力は、ネイティブスピーカの{}歳相当です。".format(user_name, aoa)
    P('<p>結果をつぶやく: ', end="")
